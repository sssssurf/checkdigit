...
本文和 `README.md` 里的命令输出都是**写死的文本**,代码一改就可能对不上。所以有一条
自动校验:
...
1. `cmd/demo` 的输出与本文记录的 56 行输出
2. `README.md` 里记录的 CLI 输出块
3. 两份 README 声称的测试数,与 `moon test` 实际报告的数字
4. `README.md` 命令列表里的 16 条命令是否都能成功退出

期望输出:

```text
RESULT: documentation matches the code.
```

**这条检查是会失败的**。把本文里任意一行输出改掉再运行,它会指出具体是哪一行、文档写的
和实际跑出来的分别是什么。CI 也跑这一步,所以文档漂移会让构建变红,而不是静静留在仓库里。
